...
### Requirement: Tapping a Past Workout opens its read-only detail view
The system SHALL open the Active Workout screen for the tapped Workout in its read-only rendering mode when a row in the Past Workouts list is tapped.

#### Scenario: Tap a row to view details
- **WHEN** the user taps a row in the Past Workouts list
- **THEN** the app SHALL open that Workout's detail view showing its Exercise Entries and Sets, with no input or action controls
