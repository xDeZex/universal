...
- [x] 2.2 `softprops/action-gh-release` step's `files` input includes both `app/build/app/outputs/flutter-apk/Universal.apk` and the new `.sha256` file
...
- [x] 4.2 Push a commit touching only `app/**` to a test branch/PR and confirm `test` runs; confirm on a simulated/manual review that `build-and-release`'s conditions would evaluate true after merge to `main`
- [x] 4.3 Confirm via code review (no live push needed) that a commit touching only `services/**` or `deploy/**` would leave both `test` and `build-and-release` skipped
